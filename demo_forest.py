...
def replace_values(series, mean):
    series = mean
    return series
...
def iforests_model(data, columns):
    ...
    # mean = pd.DataFrame(est_res.loc[:, est_res.columns].mean()).rename(columns={0: "mean"}).sort_values(by="mean", axis=0, ascending=False)
    mean = est_res.loc[:, est_res.columns].mean()
    print("每个特征名称的均值是：\n{}".format(mean))

    """
    需求：将对应的平均数mean,替换结果为-1，所对应的原来的学习数据，五个字段的值。
    """
    replace_index = data["forest"] == -1
    data.loc[replace_index, columns_yd] = data.loc[replace_index, columns_yd].apply(func=replace_values, axis=1, args=(mean,))
